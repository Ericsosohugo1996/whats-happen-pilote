...
<div id="splash-screen" class="splash-screen">
  ...
  <div id="intro-screen" class="intro-screen hidden">
    <div class="intro-bubble">Tu veux revoir tes souvenirs, ou découvrir de nouveaux lieux autour de toi&nbsp;?</div>
     <div class="intro-logo">
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
        <g transform="translate(100,90)">
          <path d="M0 -78 C -50 -78 -82 -42 -82 -2 C -82 38 -35 70 -10 90 L0 98 L10 90 C 35 70 82 38 82 -2 C 82 -42 50 -78 0 -78 Z" fill="#F2A340"/>
          <circle cx="-26" cy="-18" r="10" fill="#14213D"/>
          <circle cx="26" cy="-18" r="10" fill="#14213D"/>
          <path d="M-28 14 C -16 32, 16 32, 28 14" fill="none" stroke="#14213D" stroke-width="7" stroke-linecap="round"/>
        </g>
    ...
    <div class="intro-pills">
      <button type="button" class="intro-pill primary" id="intro-btn-souvenirs">📖 Voir mes souvenirs</button>
      <button type="button" class="intro-pill ghost" id="intro-btn-discover">📍 Découvrir des événements</button>
      <div class="intro-tagline">Parce que chaque lieu peut devenir un souvenir…</div>
    </div>
  </div>
...
<div class="flags-row" style="margin-right:6px;">
  ...
  <button class="flag-btn" id="flag-de" data-lang="de"><img src="https://flagcdn.com/w40/de.png" alt="Deutsch"></button>
    ...
    <button class="btn-ghost" id="btn-account" style="margin-right:6px;">👤</button>
    <button class="btn-ghost" id="btn-publish-header">＋ Publier</button>
  </header>
...
<section id="city-info" class="city-info"></section>
   ...
   <div class="segmented">
        <button class="seg" data-mode="carte">🗺️ Carte</button>
        <button class="seg active" data-mode="liste">☰ Liste</button>
      </div>  
...
<div class="filters-row">
  ...
  <main id="view-detail" class="view hidden">
    <button class="back-btn" id="btn-back-detail">← Retour</button>
    ...
    <div class="cta-row">
      ...
      <div class="action-tiles">
        <button class="action-tile" id="btn-favorite"><span class="tile-icon" id="favorite-icon">🤍</span><span class="tile-label">Favori</span></button>
        ...
        <button class="action-tile" id="btn-add-souvenir-detail"><span class="tile-icon">📸</span><span class="tile-label">Souvenir</span></button>
      </div>       
    </div>
  </main>
  <!-- ===================== VUE : PUBLICATION ===================== -->
  <main id="view-publish" class="view hidden">
    <button class="back-btn" id="btn-back-publish">← Retour</button>
    <h1 class="form-title" data-i18n="Nouvel événement">Nouvel événement</h1>
    <p class="form-sub">Ce pilote enregistre votre événement localement dans votre navigateur (démonstration). La publication en ligne partagée arrivera à l'étape suivante du projet.</p>
    <form id="publish-form" class="publish-form">
      <label class="field">
        <span>Titre</span>
        <input type="text" name="title" placeholder="Ex. Marché nocturne provençal" required>
      </label>
      <label class="field">
        <span>Catégorie</span>
       <label class="field">
...
<option value="caen">Caen</option>
      ...
      <label class="field">
        ...
        <input type="text" name="place" placeholder="Adresse ou nom du lieu" required>
      </label>
      <label class="field">
        <span>Description</span>
        <textarea name="description" rows="3" placeholder="Quelques mots sur l'événement…"></textarea>
      </label>
 ...
 <h1 data-i18n="Événement publié !">Événement publié !</h1>
  <p>Il apparaît maintenant dans la liste de sa ville (démonstration locale à votre navigateur).</p>
...
<button class="nav-item" id="nav-carnet-btn"><span class="dot">📖</span><span class="nav-label" style="font-weight:800;">Souvenirs</span></button>
  </nav>
  <!-- ===================== VUE : FAVORIS ===================== -->
    <main id="view-favorites" class="view hidden">
    <h1 class="form-title favorites-title" data-i18n="❤️ Mes favoris">❤️ Mes favoris</h1>
    <button type="button" id="btn-invite-friend" style="display:block; width: calc(100% - 32px); box-sizing: border-box; margin: 14px 16px; padding: 14px 20px; background: linear-gradient(135deg, #6C5CE7, #4834b0); color: #ffffff; border: none; border-radius: 14px; font-weight: 700; font-size: 15px; text-align: center; box-shadow: 0 4px 10px rgba(0,0,0,0.15); cursor: pointer;">📤 Inviter un ami (il reçoit 10 pts de bienvenue)</button>
  ...
  <main id="view-photos" class="view hidden">
    <button class="back-btn" id="btn-back-photos">← Retour</button>
    
  <h1 class="form-title photos-title" id="photos-title">📸 Mes photos</h1>
    <p class="form-sub">Ces photos restent sur cet appareil, dans ce navigateur — elles ne sont pas partagées avec les autres utilisateurs pour le moment.</p>
    <input type="file" id="photo-input" accept="image/*" multiple class="hidden">
    <button type="button" class="btn-primary btn-block" id="btn-add-photo">＋ Ajouter une photo</button>
    <div id="photos-grid" class="photos-grid"></div>
    <p id="photos-empty" class="empty-state hidden">Pas encore de souvenir ici. Ajoute une photo pour commencer !</p>
  </main>
  ...
  <div id="photo-lightbox" class="photo-lightbox hidden">
    ...
    <button type="button" class="lightbox-delete" id="btn-lightbox-delete">🗑️ Supprimer cette photo</button>
  </div>
...
<script defer src="app.js"></script>
 ...
 <script defer src="landing-mode.js"></script>
